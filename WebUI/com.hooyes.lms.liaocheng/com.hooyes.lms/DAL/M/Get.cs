...
namespace com.hooyes.lms.DAL.M
{
    public class Get : BaseGet
    {
        ...
        public static List<M1> M1_All(int PageSize, int CurrentPage, string Filter, out int Count)
        {
            var l = new List<M1>();
            try
            {
                ...
                SqlDataReader dr = SqlHelper.ExecuteReader(SqlHelper.Local, CommandType.StoredProcedure, "M_Get_MemberList_All", param);
                while (dr.Read())
                {
                    var m = new M1();
                    m.ID = Convert.ToInt32(dr["ID"]);
                ...
                var oCount = SqlHelper.ExecuteScalar(SqlHelper.Local, CommandType.StoredProcedure, "M_Get_MemberListCount_all", param2);
