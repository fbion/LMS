...
namespace com.hooyes.lms.DAL
{
    public class Task
    {
        ...
        /// <returns></returns>
        public static R EvaluteReport(int MID, int PID, int RegionCode)
        {
            //log.Info("EvaluteContents:{0},{1}", MID, CID);
            var m = new R();
            try
            {
                ...
                var r = SqlHelper.ExecuteNonQuery(SqlHelper.Local, CommandType.StoredProcedure, "Task_EvaluteReport", param);
                m.Code = 0;
                m.Value = 0;
            }
            catch (Exception ex)
