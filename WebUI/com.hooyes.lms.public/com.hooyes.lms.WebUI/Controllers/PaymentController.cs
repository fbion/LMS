...
namespace com.hooyes.lms.Controllers
{
    ...
    public class PaymentController : Controller
    {
        ...
        public ActionResult Buy(int ID = -1)
        {
            ...
            if (ID == -1)
            {
                Products = DAL.Get.Products(Client.MID);
            }
            else
            {
                Products = DAL.Get.ProductsByCate(Client.MID, ID);
            }
        ...
        {
            var Orders = DAL.Get.Order(Client.MID);
        ...
        public ActionResult Orders(int ID, int Refresh = 0)
        {
            ...
            if (Order.Status < 10 && Order.ID > 0)
            {
                ...
                var Products = DAL.Get.Products(Client.MID,Order.Tags);
                ViewData["Products"] = Products.FindAll(n => n.MyID <= 0);
        ...
        {
            var Order = DAL.Get.Order(Client.MID, ID);
            var Products = DAL.Get.Products(Order.Tags);
            ViewData["Products"] = Products;
            ViewData["Order"] = Order;
            return View();
        }

        public ActionResult OrdersView(int ID, int Refresh = 0)
        {
        ...
        {
            var Balance = DAL.Get.Balance(Client.MID);
            var MyCards = DAL.Get.MyCards(Client.MID);
            var Transactions = DAL.Get.Transactions(Client.MID);
            ViewData["Balance"] = Balance;
            ViewData["MyCards"] = MyCards;
