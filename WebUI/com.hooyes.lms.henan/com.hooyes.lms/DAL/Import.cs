    ...
    public class Import
    {
        ...
        public static R CoursesSavedb(Courses courses, List<Contents> contents)
        {
            ...
            try
            {
                ...
                string SQL2 = @"INSERT INTO [Contents]([CName],[CCID],[CCName],[Name],[Duration],[Url],[MIME])
                ...
                {
                    SQL.AppendLine();
                    SQL.AppendFormat(SQL2, c.CName, c.CCID, c.CCName, c.Name, c.Duration, c.Url, c.MIME);
                }
                SQL.AppendLine();
                SQL.Append("END");
                log.Info(SQL.ToString());
        ...
        public static R MessageQueue(string excelPath, int MsgID)
        {
            var m = new R();
            try
            {
                string SQL = "Select Phone from [sheet1$]";
                var dr = excel.ExcuteReader(excelPath, SQL);
                while (dr.Read())
                {
                    ...
                    try
                    {
                            ...
                            SqlParameter[] param =
                        {
                            ...
                            new SqlParameter("@Phone",dr["Phone"].ToString())
                        };
                            param[0].Direction = ParameterDirection.ReturnValue;
                            param[1].Direction = ParameterDirection.Output;
                            param[2].Direction = ParameterDirection.Output;

                            var r = SqlHelper.ExecuteNonQuery(C.conn, CommandType.StoredProcedure, "M_Import_MessageQueue", param);
                            log.Info("M_Import_MessageQueue:{0},{1},{2}", param[0].Value, param[1].Value, param[2].Value);
                        }
                    }
                    catch (Exception ex1)
                    {
